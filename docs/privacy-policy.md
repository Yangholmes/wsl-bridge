...
- 本应用为**完全本地运行**的桌面应用，核心数据仅存储在您的设备上
- 本应用使用 **Microsoft Clarity** 收集匿名使用统计数据（详见下文）
- Clarity 仅收集匿名标识符、应用版本和下载渠道，**不收集**网络流量内容或个人身份信息
- 您可以在设置中查看 Clarity 的数据收集详情
...
- [使用统计：Microsoft Clarity](#使用统计microsoft-clarity)
- [权限说明](#权限说明)
- [儿童隐私](#儿童隐私)
- [本隐私声明的变更](#本隐私声明的变更)
- [如何联系我们](#如何联系我们)
...
### 使用统计数据（Microsoft Clarity）

本应用使用 Microsoft Clarity 收集匿名使用统计数据，用于改进产品体验：

| 数据类型       | 说明                                    | 用途               |
| -------------- | --------------------------------------- | ------------------ |
| **匿名用户 ID** | 首次启动时生成的随机 UUID               | 区分不同用户会话   |
| **应用版本**   | 当前安装的应用版本号                    | 版本分布统计       |
| **下载渠道**   | 应用来源渠道（如 GitHub、Microsoft Store） | 渠道效果分析       |
| **页面访问**   | 用户访问的应用页面                      | 了解用户使用路径   |

**Clarity 数据特点**：

- 用户 ID 为**随机生成**，不关联任何个人身份信息
- 数据上传至 Microsoft Clarity 服务器（美国）
- 不收集网络流量内容、文件内容或任何敏感数据
- 符合 GDPR 和 CCPA 要求
- Clarity 隐私政策：https://privacy.microsoft.com/en-us/privacystatement

**如何禁用 Clarity**：

目前 Clarity 为强制启用，未来版本将提供禁用选项。如需完全禁用，可：
- 阻止应用访问 `clarity.ms` 域名（通过防火墙规则）
- 使用开发版构建（开发模式下 Clarity 不启用）
...
- 规则配置、防火墙策略、MCP 配置均本地存储
- 不连接远程服务器（除 Clarity 外）
- 不上传云端
- 不与其他设备同步

**使用统计数据（Clarity）**：

- 上传至 Microsoft Clarity 服务器（美国）
- 仅包含匿名标识符、版本号和渠道信息
- 不包含任何敏感或个人身份信息
...
## 使用统计：Microsoft Clarity

Microsoft Clarity 是本应用集成的使用统计工具，帮助我们了解用户如何使用应用，以便改进产品体验。
...
| 数据           | 说明                                 |
| -------------- | ------------------------------------ |
| 用户会话 ID    | 随机生成的 UUID，不关联个人身份       |
| 应用版本       | 您当前使用的应用版本号               |
| 下载渠道       | 应用来源（GitHub、Microsoft Store等） |
| 页面访问路径   | 您在应用内访问的页面                 |
| 交互行为       | 点击、滚动等基本交互（热力图数据）   |
...
- ❌ 规则配置或防火墙设置
- ❌ WSL/Hyper-V 配置或拓扑信息
- ❌ MCP 服务器配置或 API Token
- ❌ 姓名、电子邮件或其他个人身份信息
- ❌ 设备序列号、MAC 地址或硬件标识

### 数据去向

- Clarity 数据上传至 Microsoft 服务器（美国）
- 数据存储在 Microsoft Azure 基础设施
- Microsoft 遵守 GDPR、CCPA 等隐私法规

### Microsoft 隐私政策

Clarity 的数据处理遵循 Microsoft 隐私政策：
