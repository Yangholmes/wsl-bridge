...
8. 需要同时支持标准权限版与管理员版两种 Windows 构建口径。
...
- 前端允许管理配置态规则：
  - 新建、编辑、删除规则
  - 启用/禁用非 `hyperv` 规则
  - 应用不依赖 `hyperv` 和防火墙配置的规则
- 不允许：
  - 编辑防火墙配置
  - 启用/禁用 `hyperv` 规则
  - 创建或管理依赖管理员权限的 Hyper-V 目标规则
- 页面顶部显示权限提示。

2. `su`

- 通过 `pnpm tauri build --su` 构建。
- 最终 EXE 嵌入 `requireAdministrator` manifest。
- 启动时触发 UAC。
- 显示全部功能。

发布策略：

- GitHub Release：仅发布 `su` 版完整功能安装包。
- 标准权限版：保留给本地构建或后续商店侧发行链路使用。

### 3.3 对 `netsh interface portproxy` 的结论
...
1. 用户启动 `wsl-bridge.exe`。
2. 应用初始化数据库与运行时。
3. 应用检测当前是否具备管理员权限与当前构建口径。
4. 加载规则与拓扑信息并展示。
5. 若当前不是管理员权限，则前端进入“配置态可编辑、运行态受限”的标准权限模式，并展示提示条。
6. 用户点击“应用规则”后执行 `apply_rules()`。
...
- 前端按权限状态细分能力边界：
  - 标准权限下保留配置态规则管理
  - 标准权限下允许应用不依赖 `hyperv` 和防火墙配置的规则
  - 防火墙配置与 Hyper-V 管理能力受限
...
2. 在管理员版下，可创建并应用 TCP/UDP 规则，外部能访问 WSL/Hyper-V 服务。
